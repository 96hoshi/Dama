...
#   checks if the enemy have no moves to perform
    enemy_color = opposite_color(color)

    enemy_legal = board_legal_moves(enemy_color, board)
    if not check_if_empty(enemy_legal):
        return False

    enemy_forced = board_forced_moves(enemy_color, board)
    if not check_if_empty(enemy_forced):
        return False

#   check if there is only player color on the board
    for i in range(SIZE):
        for j in range(SIZE):
            if board[i][j] == EMPTY:
                continue
            if color_check(i, j, color, board):
                continue
            if color_check(i, j, enemy_color, board):
                return False
...
# removing enemy pedina if needed
...
# check if is possible to updates the board moving a pedina.
